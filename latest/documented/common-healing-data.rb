module Lich
  # Provides mappings and regex patterns for health-related data in the DragonRealms game.
  # @example Usage
  ...
  module DragonRealms
    ...
    # Maps bleed rates from the `health` command to severity numbers.
    # A partially tended wound is considered more severe than its non-tended counterpart.
    # @example Accessing severity
    #   severity = $DRCH_BLEED_RATE_TO_SEVERITY_MAP['slight'][:severity]
    ...
    # @example Accessing severity
    #   severity = $DRCH_LODGED_TO_SEVERITY_MAP['shallowly']
    ...
    # List of regex patterns for identifying parasites in health descriptions.
    # @example Accessing regex
    #   regex = $DRCH_PARASITES_REGEX_LIST[0]
    ...
    # Regex pattern to parse the severity number from the wound line in 'perceive health self'.
    # @example Using regex
    #   match = 'Fresh External: light scratches -- negligible'.match($DRCH_PERCEIVE_HEALTH_SEVERITY_REGEX)
    ...
    # Regex pattern to match body parts in health descriptions.
    # @example Using regex
    #   match = 'on your left arm'.match($DRCH_BODY_PART_REGEX)
    ...
    # @example Accessing severity
    #   severity_info = $DRCH_WOUND_SEVERITY_REGEX_MAP[/minor abrasions to the left arm/]
    ...
    # https://elanthipedia.play.net/Damage#Wounds
    # Regex pattern to separate wound descriptions by commas.
    # @example Using regex
    #   parts = 'swollen, bruised, scarred'.split($DRCH_WOUND_COMMA_SEPARATOR)
