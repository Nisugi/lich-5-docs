...
module Lich
  module DragonRealms
    # Parses server output for DragonRealms
    # @example Using the DRParser module
    #   DRParser.parse(server_output)
    module DRParser
      module Pattern
        # Regular expression for experience columns
        # Matches skill, rank, percent, and rate.
        ExpColumns = /(?:\s*(?<skill>[a-zA-Z\s]+)\b:\s*(?<rank>\d+)\s+(?<percent>\d+)%\s+(?<rate>[a-zA-Z\s]+)\b)/.freeze
        # Regular expression for brief experience on
        # Matches skill, rank, percent, and rate in brief experience output.
        BriefExpOn = %r{<component id='exp .*?<d cmd='skill (?<skill>[a-zA-Z\s]+)'.*:\s+(?<rank>\d+)\s+(?<percent>\d+)%\s*\[\s?(?<rate>\d+)\/34\].*?<\/component>}.freeze
        # Regular expression for brief experience off
        # Matches skill, rank, percent, and rate in brief experience output.
        BriefExpOff = %r{<component id='exp .*?\b(?<skill>[a-zA-Z\s]+)\b:\s+(?<rank>\d+)\s+(?<percent>\d+)%\s+\b(?<rate>[a-zA-Z\s]+)\b.*?<\/component>}.freeze
        # Regular expression for name, race, and guild
        # Matches name, race, and guild information.
        NameRaceGuild = /^Name:\s+\b(?<name>.+)\b\s+Race:\s+\b(?<race>.+)\b\s+Guild:\s+\b(?<guild>.+)\b\s+/.freeze
        # Regular expression for gender, age, and circle
        # Matches gender, age, and circle information.
        GenderAgeCircle = /^Gender:\s+\b(?<gender>.+)\b\s+Age:\s+\b(?<age>.+)\b\s+Circle:\s+\b(?<circle>.+)/.freeze
        # Regular expression for stat values
        # Matches various stats and their corresponding values.
        StatValue = /(?<stat>Strength|Agility|Discipline|Intelligence|Reflex|Charisma|Wisdom|Stamina|Favors|TDPs)\s+:\s+(?<value>\d+)/.freeze
        # Regular expression for TDP values
        # Matches the number of TDPs available.
        TDPValue = /You have (\d+) TDPs\./.freeze
        # Regular expression for encumbrance values
        # Matches the encumbrance status.
        EncumbranceValue = /^\s*Encumbrance\s+:\s+(?<encumbrance>[\w\s'?!]+)$/.freeze
        # Regular expression for luck values
        # Matches the luck status.
        LuckValue = /^\s*Luck\s+:\s+.*\((?<luck>[-\d]+)\/3\)/.freeze
        # Regular expression for balance values
        # Matches the balance status.
        BalanceValue = /^(?:You are|\[You're) (?<balance>#{Regexp.union(DR_BALANCE_VALUES)}) balanced?/.freeze
        # Regular expression for clearing mindstate experience
        # Matches skill information for clearing mindstate.
        ExpClearMindstate = %r{<component id='exp (?<skill>[a-zA-Z\s]+)'><\/component>}.freeze
        # Regular expression for room players
        # Matches players present in the room.
        RoomPlayers = %r{\'room players\'>Also here: (.*)\.</component>}.freeze
        # Regular expression for empty room players
        # Matches when no players are present in the room.
        RoomPlayersEmpty = %r{\'room players\'></component>}.freeze
        # Regular expression for room objects
        # Matches objects present in the room.
        RoomObjs = %r{\'room objs\'>(.*)</component>}.freeze
        # Regular expression for empty room objects
        # Matches when no objects are present in the room.
        RoomObjsEmpty = %r{\'room objs\'></component>}.freeze
        # Regular expression for group members
        # Matches members of the player's group.
        GroupMembers = %r{<pushStream id="group"/>  (\w+):}.freeze
        # Regular expression for empty group members
        # Matches when no group members are present.
        GroupMembersEmpty = %r{<pushStream id="group"/>Members of your group:}.freeze
        # Regular expression for the start of experience modifiers
        # Matches the beginning of experience modifiers output.
        ExpModsStart = /^(<.*?\/>)?The following skills are currently under the influence of a modifier/.freeze
        # Regular expression for known spells start
        # Matches the beginning of known spells output.
        KnownSpellsStart = /^You recall the spells you have learned/.freeze
        # Regular expression for barbarian abilities start
        # Matches the beginning of known barbarian abilities output.
        BarbarianAbilitiesStart = /^You know the (Berserks:)/.freeze
        # Regular expression for thief khri start
        # Matches the beginning of known thief khri output.
        ThiefKhriStart = /^From the Subtlety tree, you know the following khri:/.freeze
        # Regular expression for spell book format
        # Matches the format of the spell book output.
        SpellBookFormat = /^You will .* (?<format>column-formatted|non-column) output for the SPELLS verb/.freeze
        # Regular expression for played account
        # Matches account information.
        PlayedAccount = /^(?:<.*?\/>)?Account Info for (?<account>.+):/.freeze
        # Regular expression for played subscription
        # Matches the subscription status.
        PlayedSubscription = /Current Account Status: (?<subscription>F2P|Basic|Premium|Platinum)/.freeze
        # Regular expression for last logoff
        # Matches the last logoff time and date.
        LastLogoff = /^\s+Logoff :  (?<weekday>[A-Z][a-z]{2}) (?<month>[A-Z][a-z]{2}) (?<day>[\s\d]{2}) (?<hour>\d{2}):(?<minute>\d{2}):(?<second>\d{2}) ET (?<year>\d{4})/.freeze
        # Regular expression for room ID off
        # Matches when room IDs are turned off.
      ...
      # @param server_string [String] The server output string
      # @return [String] The original server string
      # @example
      #   check_events(server_output)
      ...
      # @return [String] The original server string
      # @example
      #   check_exp_mods(server_output)
      # @note This method expects specific output format.
      ...
      # @return [String] The original server string
      # @example
      #   check_known_spells(server_output)
      # @note This method handles both column-formatted and non-column output.
      ...
      # @return [String] The original server string
      # @example
      #   check_known_barbarian_abilities(server_output)
      ...
      # @return [String] The original server string
      # @example
      #   check_known_thief_khri(server_output)
      ...
      # @param line [String] The server output line
      # @return [void]
      # @example
      #   parse(server_output_line)
