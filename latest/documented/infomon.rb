...
module Lich
  module Gemstone
    # Replacement for the venerable infomon.lic script used in Lich4 and Lich5.
    # Supports Ruby 3.X builds.
    # @example Usage
    #   Lich::Gemstone::Infomon.get('some_key')
    module Infomon
      ...
      # Returns the database connection for Infomon.
      # @return [Sequel::Database] The database connection.
      def self.db
        @db
      end

      # Returns the mutex used for thread safety in Infomon.
      ...
      # Generates the table name based on the game and XMLData name.
      ...
      # Returns the dataset for the Infomon table, setting it up if necessary.
      # @return [Sequel::Dataset] The Infomon table dataset.
      def self.table
        @_table ||= self.setup!
      end
      ...
      # Loads the cache from the database if not already loaded.
      # @return [void]
      def self.cache_load
      ...
      # Normalizes the key for storage in the database.
      ...
      # Normalizes the value for storage in the database.
      ...
      # @constant [Array<Class>] Allowed types for Infomon values.
      ...
      # @return [Symbol] :noop if the value is unchanged, otherwise performs the insert.
      ...
      # Thread that processes SQL statements from the queue asynchronously.
      Thread.new do
        loop do
