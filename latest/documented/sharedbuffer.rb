...
module Lich
  module Common
    # A thread-safe buffer for sharing data between threads.
    # This class allows multiple threads to read from and write to a shared buffer.
    ...
    class SharedBuffer
      ...
      # Clears the buffer and returns the lines that were removed.
      # @return [Array<String>] An array of lines that were in the buffer.
      def clear
        thread_id = Thread.current.object_id
        if @buffer_index[thread_id].nil?
          @buffer_mutex.synchronize { @buffer_index[thread_id] = (@buffer_offset + @buffer.length) }
      ...
      # Resets the buffer index for the current thread to the start of the buffer.
      ...
      # Adds a new line to the buffer, maintaining the maximum size.
