module Lich
  ...
  # This module manages the state of claimed rooms and handles synchronization.
  ...
  module Claim
    ...
    @last_room    ||= nil
    @mine         ||= false
    # Buffer for storing temporary data related to claims.
    @buffer         = []
    # List of other characters in the room.
    @others         = []
    # Timestamp of the last claim action.
    @timestamp      = Time.now
    ...
    # Acquires the lock for the Claim module.
    # @return [void]
    # @note This method will only lock if the lock is not already owned.
    def self.lock
      Lock.lock if !Lock.owned?
    end

    # Releases the lock for the Claim module.
    # @return [void]
    # @note This method will only unlock if the lock is currently owned.
    def self.unlock
      Lock.unlock if Lock.owned?
    end

    # Checks if the current instance is the one that claimed the room.
    # @return [Boolean] True if this instance is the owner of the claim, false otherwise.
    def self.current?
    ...
    # @return [Array<String>] An array of character names that are not claimed.
    def self.others
      @others
    end
