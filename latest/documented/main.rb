...
@main_thread = Thread.new {
  ...
  # @example Checking for login argument
  ...
  # Limited array for client buffer
  ...
  # @example Checking for launch file
  #   if @argv_options[:sal]
  if @argv_options[:sal]
    begin
  ...
  # Checks if launch data is available
  # @example Checking for launch data
  #   if @launch_data
  if @launch_data
    ...
    Lich.log 'info: connected'
  # Handles connection when game host and port are defined
  # @example Connecting with defined host and port
  ...
  # @example Checking frontend compatibility
