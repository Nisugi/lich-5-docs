# Manual login screen for the Lich5 GUI.
# This file is intended to load as part of the login GUI Gtk queue block method.
...
# List store for managing game and character data.
...
# Radio button for selecting the Wrayth frontend.
...
# Check button for enabling custom launch commands.
...
# Connects a signal to toggle the visibility of custom launch options.
...
# Connects the click event of the connect button.
# @note This method handles user authentication and updates the UI accordingly.
...
# Connects the click event of the disconnect button.
...
# Connects the click event of the play button.
# @note This method handles launching the game with the selected character.
...
# Connects the activate event of the user ID entry to focus on the password entry.
user_id_entry.signal_connect('activate') {
  pass_entry.grab_focus
}

# Connects the activate event of the password entry to trigger the connect button.
