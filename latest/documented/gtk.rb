...
module Lich
  # The Common module
  # This module contains common functionality used throughout the Lich application.
  module Common
    if defined?(Gtk)
      ...
      # Calling Gtk API in a thread other than the main thread may cause random segfaults.
      # @param block [Proc] The block of code to execute in the GTK main thread.
      ...
      # @raise [ThreadError] If a thread-related error occurs.
      # @raise [SystemStackError] If the stack level is too deep.
      # @raise [LoadError] If a required file cannot be loaded.
      # @raise [NoMemoryError] If there is not enough memory available.
      # @example Queuing a block
      #   Gtk.queue { puts 'Hello from GTK thread!' }
      def Gtk.queue(&block)
        GLib::Timeout.add(1) {
          begin
            block.call
    ...
    # Checks if the logo.png file exists and creates it if it does not.
    unless File.exist?('logo.png')
      # Creates a logo.png file with a default image if it does not exist.
    ...
    begin
      # Queues the initialization of the default icon and theme state in the GTK main thread.
      Gtk.queue {
        # Initializes the default icon using the logo.png file.
        @default_icon = GdkPixbuf::Pixbuf.new(:file => 'logo.png')
        # Adds a function to be called when GTK is idle.
        # Add a function to call for when GTK is idle
        GLib::Idle.add do
          sleep 0.01
        end
        # Tracks the current dark mode state for the application.
