# The Lich module contains various constants and data structures for the DragonRealms game.
module Lich
  # The DragonRealms module holds constants related to skills, learning rates, and other game mechanics.
  # @example Accessing learning rates
  #   Lich::DragonRealms::DR_LEARNING_RATES
  module DragonRealms
    # An array of learning rates used in the game.
    # @example Accessing a learning rate
    #   Lich::DragonRealms::DR_LEARNING_RATES[0] # => 'clear'
    DR_LEARNING_RATES = [
      'clear',
      'dabbling',
      'perusing',
    ...
    # An array of balance values used in the game.
    # @example Accessing a balance value
    #   Lich::DragonRealms::DR_BALANCE_VALUES[0] # => 'completely'
    DR_BALANCE_VALUES = [
      'completely',
      'hopelessly',
      'extremely',
    ...
    # A hash containing skillsets and their respective skills in the game.
    # @example Accessing skills for Armor
    #   Lich::DragonRealms::DR_SKILLS_DATA[:skillsets][:Armor]
    DR_SKILLS_DATA = {
      "skillsets": {
        "Armor": [
          "Shield Usage",
    ...
    # An array of bank names in the Kronar region.
    # @example Accessing a Kronar bank
    #   Lich::DragonRealms::KRONAR_BANKS[0] # => 'Crossings'
    KRONAR_BANKS = ['Crossings', 'Dirge', 'Ilaya Taipa', 'Leth Deriel']
    # An array of bank names in the Lirum region.
    # @example Accessing a Lirum bank
    #   Lich::DragonRealms::LIRUM_BANKS[0] # => 'Aesry Surlaenis'a'
    LIRUM_BANKS = ["Aesry Surlaenis'a", "Hara'jaal", "Mer'Kresh", "Muspar'i", "Ratha", "Riverhaven", "Rossman's Landing", "Therenborough", "Throne City"]
    # An array of bank names in the Dokora region.
    # @example Accessing a Dokora bank
    #   Lich::DragonRealms::DOKORA_BANKS[0] # => 'Ain Ghazal'
    DOKORA_BANKS = ["Ain Ghazal", "Boar Clan", "Chyolvea Tayeu'a", "Hibarnhvidar", "Fang Cove", "Raven's Point", "Shard"]

    # A hash mapping bank names to their titles in the game.
    # @example Accessing titles for a bank
    #   Lich::DragonRealms::BANK_TITLES['Crossings']
    BANK_TITLES = {
    ...
    # A hash mapping vault names to their titles in the game.
    # @example Accessing titles for a vault
    #   Lich::DragonRealms::VAULT_TITLES['Crossings']
    VAULT_TITLES = {
    ...
    # A constant representing an unknown duration for spells.
    # @example Using UNKNOWN_DURATION
    #   duration = Lich::DragonRealms::UNKNOWN_DURATION
    UNKNOWN_DURATION = 1000 unless defined?(UNKNOWN_DURATION)

    # A hash mapping hometown names to their regex patterns for matching.
    # @example Accessing regex for a hometown
    #   Lich::DragonRealms::$HOMETOWN_REGEX_MAP['Crossing']
    $HOMETOWN_REGEX_MAP = {
    ...
    # A list of canonical town names in the game.
    # @example Accessing the hometown list
    #   Lich::DragonRealms::$HOMETOWN_LIST
    $HOMETOWN_LIST = $HOMETOWN_REGEX_MAP.keys

    # Union of regular expressions that match town names, like /^(theren(borough)?)$/i
    # A union of regex patterns that match town names.
    # @example Using HOMETOWN_REGEX
    #   if input =~ Lich::DragonRealms::$HOMETOWN_REGEX
    $HOMETOWN_REGEX = Regexp.union($HOMETOWN_REGEX_MAP.values)

    # An array of ordinal numbers as strings.
    # @example Accessing ordinals
    #   Lich::DragonRealms::$ORDINALS[0] # => 'first'
    ...
    # An array of currency types used in the game.
    # @example Accessing currencies
    #   Lich::DragonRealms::$CURRENCIES[0] # => 'Kronars'
    $CURRENCIES = %w[Kronars Lirums Dokoras]

    # A hash mapping encumbrance descriptions to their respective values.
    # @example Accessing encumbrance value
    #   Lich::DragonRealms::$ENC_MAP['Light Burden'] # => 1
    $ENC_MAP = {
      'None'                                => 0,
      'Light Burden'                        => 1,
      'Somewhat Burdened'                   => 2,
    ...
    # A hash mapping number words to their respective integer values.
    # @example Accessing number value
    #   Lich::DragonRealms::$NUM_MAP['two'] # => 2
    $NUM_MAP = {
      'zero'      => 0,
      'one'       => 1,
      'two'       => 2,
    ...
    # A regex pattern for matching box types in the game.
    # @example Using box regex
    #   if input =~ Lich::DragonRealms::$box_regex
    ...
    # A hash mapping mana states to their respective descriptions.
    # @example Accessing mana states
    #   Lich::DragonRealms::$MANA_MAP['good']
    $MANA_MAP = {
    ...
    # A regex pattern for matching primary sigils in the game.
    # @example Using primary sigils pattern
    #   if input =~ Lich::DragonRealms::$PRIMARY_SIGILS_PATTERN
    $PRIMARY_SIGILS_PATTERN = /\b(?:abolition|congruence|induction|permutation|rarefaction) sigil\b/
    # A regex pattern for matching secondary sigils in the game.
    # @example Using secondary sigils pattern
    #   if input =~ Lich::DragonRealms::$SECONDARY_SIGILS_PATTERN
    ...
    # A hash mapping volume descriptions to their respective values.
    # @example Accessing volume value
    #   Lich::DragonRealms::$VOL_MAP['large'] # => 4
    $VOL_MAP = {
      'enormous' => 20,
      'massive'  => 10,
      'huge'     => 5,
